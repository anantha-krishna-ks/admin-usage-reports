...
export const SectionDrillDown = () => {
  ...
  const [previewSection, setPreviewSection] = useState<{ grade: string; section: string } | null>(null);
  ...
  const previewKey = previewSection ? `${previewSection.grade}-${previewSection.section}` : null;
  const previewUsers = previewKey ? userDetailsBySection[previewKey] || [] : [];
                    ...
                    <TableRow key={grade.grade} className="cursor-pointer hover:bg-muted/50" onClick={() => toggleGrade(grade.grade)}>
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setPreviewSection({ grade: grade.grade, section: grade.sections[0]?.name || "" });
                          }}
                        >
                            ...
                            <TableRow key={sectionKey} className="bg-muted/20 cursor-pointer hover:bg-muted/40" onClick={() => section.subSections && toggleSection(sectionKey)}>
                              ...
                              <TableCell>
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setPreviewSection({ grade: grade.grade, section: section.name });
                                  }}
                                >
                                  <Eye className="h-4 w-4" />
                                </Button>
        ...
        <DialogContent className="max-w-4xl max-h-[85vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold">
              {previewSection?.grade} - {previewSection?.section} — User Details
            </DialogTitle>
            <DialogDescription>
              Individual user usage breakdown for this section
            </DialogDescription>
          </DialogHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  ...
                  <TableHead className="font-semibold">App Usage (hrs)</TableHead>
                  <TableHead className="font-semibold">Content Usage (hrs)</TableHead>
                  <TableHead className="font-semibold">Total Usage (hrs)</TableHead>
              ...
              <TableBody>
                {previewUsers.map((user) => (
                  <TableRow key={user.userName} className="hover:bg-muted/30">
                    ...
                    <TableCell>{user.appUsage}</TableCell>
                    <TableCell>{user.contentUsage}</TableCell>
                    <TableCell className="font-semibold">{user.totalUsage}</TableCell>
